...
-- These are required by Spring Batch to track job executions
...
CREATE INDEX IF NOT EXISTS idx_job_execution_job_instance ON BATCH_JOB_EXECUTION(JOB_INSTANCE_ID);
CREATE INDEX IF NOT EXISTS idx_job_execution_status ON BATCH_JOB_EXECUTION(STATUS);
CREATE INDEX IF NOT EXISTS idx_step_execution_job_exec ON BATCH_STEP_EXECUTION(JOB_EXECUTION_ID);
CREATE INDEX IF NOT EXISTS idx_step_execution_step_name ON BATCH_STEP_EXECUTION(STEP_NAME);

-- Add sequences for batch tables (if not using BIGSERIAL)
