...
/**
 * Optimized batch configuration with strict requirements implementation
 * - OpenAI for text generation tasks
 * - Ollama for vector embeddings only
 * - Comprehensive place filtering
 * - Database initialization before processing
 * - Performance optimized with async processing
 */
@Configuration
public class OptimizedBatchConfiguration {

    private static final Logger logger = LoggerFactory.getLogger(OptimizedBatchConfiguration.class);
    ...
    public OptimizedBatchConfiguration(
     ...
     * Includes database initialization step as required
     */
     ...
     * Clears existing place data as per requirements
     */
    ...
    /**
     * Main place enrichment step with optimized processing
     */
    @Bean(name = "placeEnrichmentStep")
    public Step placeEnrichmentStep(
            JobRepository jobRepository,
            PlatformTransactionManager transactionManager,
            ItemReader<EnrichedPlace> placeReader,
        ...
        return (contribution, chunkContext) -> {
            ...
            try {
                ...
                if (result.isSuccess()) {
                    ...
                    logger.error("❌ Database initialization failed: {}", result.getMessage());
                    throw new RuntimeException("Database initialization failed: " + result.getMessage());
                }
    ...
    /**
     * Task executor for place processing with controlled concurrency
     */
    ...
    /**
     * Batch task executor for async operations
     */
